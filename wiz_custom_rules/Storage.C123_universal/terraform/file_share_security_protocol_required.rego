...
# --- Logic ---
# Universal: every share with SMB settings must include the strong Kerberos and
# Channel encryption values. Evaluated only when an smb block is present.
...
WizPolicy[result] {
    ...
    result := {
        ...
        "keyActualValue": sprintf("Kerberos encryption does not include '%v'", [required_kerberos]),
...
WizPolicy[result] {
    ...
    result := {
        ...
        "keyActualValue": sprintf("Channel encryption does not include '%v'", [required_channel]),
        "resourceTags": terraLib.get_resource_tags(resource),
    }
}
